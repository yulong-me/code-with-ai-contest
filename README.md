# **🚀 “Code with AI” 极客探索赛：5G 信号可视化看板挑战**

## **一、 比赛背景**

**“还在手写繁琐的数据处理逻辑？还在为画个漂亮的交互地图查半天文档？”**

为了在商用交付中全面推广 AI Coding Agent（智能编程助手），提升部门整体研发效能，我们特别策划了本次“Code with AI”极客探索赛！

本次比赛不以考察复杂的底层算法为目的，而是希望鼓励大家跳出传统的代码编写模式，体验如何像“产品经理”一样用自然语言指挥 AI 帮你干活。通过本活动，希望大家能快速建立对 AI 辅助编程的认知，将精力从繁复的 API 调试和 UI 编写中解放出来，聚焦于核心业务逻辑与架构设计。
...
* **活动周期**：**为期 1 周**（即日起至下周 X 晚 24:00 截止）。  
* **组队形式**：自由结对，**3 人一组**。  
* **赛事奖励**：本次活动奖励主打“先到先得”。前 30 组成功提交合格作品（通过验收标准）的小组，将获得 **每组 300 元** 的 AI Coding Plan 专项经费（奖池发完即止）。  
* **报名方式**：请各小组队长通过此问卷链接完成报名并获取官方代码仓库地址：👉 \[填写您的问卷收集链接\] 👈

## **四、 ⚡️ 小白 5 分钟速成指南**

致习惯了底层开发的极客们：别慌！你不需要写 HTML 和 CSS。跟着以下步骤，体验魔法：

1. **装环境**：在终端运行 pip install \-r requirements.txt 安装基础库（如 streamlit, pandas 等）。  
2. **找 AI**：用 Cursor 等自带 AI 的编辑器打开你的 Python 模板文件。  
3. **念咒语**：打开 AI Chat 窗口，**不要自己敲代码**，直接输入类似这样的话：*"我正在用 Streamlit 写看板。请帮我读取当前目录下的 data/signal\_samples.csv，在网页上加个大标题，然后用 st.map() 把数据里的纬度(Latitude)和经度(Longitude)画在地图上。"*  
4. **看结果**：AI 生成代码并 Apply 后，在终端运行 streamlit run 你的文件名.py。你的浏览器会自动打开一个本地网页，见证奇迹！  
5. **不断迭代**：继续对着 AI 提出修改意见，比如 *"报错了，错误信息是XXX，帮我修一下"*，或者 *"在左边加一个滑动条，只显示筛选后的数据..."*。

**Ready? Go\! 祝各位早日进化为“AI 魔法师”！**